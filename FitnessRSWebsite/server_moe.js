...
async function handle(request, response) {
 ...
 //       console.log(request.method);
        if (request.method === 'POST') {
            var newUser = require('./createUsers_moe');
            newUser.newUserDB();
        }
    }
...
async function callImplicit(response) {
    var pyshell = new PythonShell('Implicit_moe.py', { mode: 'text' });
...
async function getLabelData(text, url, response) {
    ...
    var pyshell = new PythonShell('LabelVideo_moe.py', { mode: 'text' });
    var contentlist = [];
...
async function getMainData(text, response) {
    var pyshell = new PythonShell('Hybrid_moe.py', { mode: 'text' });
    var page='';
    text = text+'';
    var parts = text.split("$");
    var end = parts[38];
