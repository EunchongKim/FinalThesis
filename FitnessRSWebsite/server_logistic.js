...
var spawn = require('child_process').spawn,
    py    = spawn('python', ['Hybrid_logistic.py']);
...
async function handle(request, response) {
 ...
 //       console.log(request.method);
        if (request.method === 'POST') {
            var newUser = require('./createUsers_logistic');
            newUser.newUserDB();
        }
    }
...
async function callImplicit(response) {
    var pyshell = new PythonShell('Implicit_logistic.py', { mode: 'text' });
...
async function getLabelData(text, url, response) {
    ...
    var pyshell = new PythonShell('LabelVideo_logistic.py', { mode: 'text' });
    var contentlist = [];
...
async function getMainData(text, response) {
    var pyshell = new PythonShell('Hybrid_logistic.py', { mode: 'text' });
    var page='';
    text = text+'';
    var parts = text.split("$");
    var end = parts[38];
...
//    console.log(dbpos);
    ...
    var str = data.labels.split(";");
    for (var i = 0; i < labelNum; i++) {
        if (parseFloat(str[i]) >= 0.5) {
